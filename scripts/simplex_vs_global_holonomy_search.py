...
LEGACY_DOCS_DIR = ROOT / "docs" / "writing"
...
def main() -> None:
    ...
    legacy_report = LEGACY_RESULTS_DIR / "simplex_vs_global_holonomy_search.md"
    legacy_json = LEGACY_RESULTS_DIR / "simplex_vs_global_holonomy_search.json"
    legacy_svg = LEGACY_RESULTS_DIR / "simplex_vs_global_holonomy_search.svg"
    for legacy_path, target in (
        (legacy_report, report_path),
        (legacy_json, json_path),
        (legacy_svg, svg_path),
    ):
        if legacy_path.exists() or legacy_path.is_symlink():
            legacy_path.unlink()
        legacy_path.symlink_to(target.relative_to(legacy_path.parent))

    for stale_name in note_candidates:
        legacy_stale = LEGACY_DOCS_DIR / stale_name
        if legacy_stale.exists() or legacy_stale.is_symlink():
            legacy_stale.unlink()
    legacy_note = LEGACY_DOCS_DIR / note_path.name
    legacy_note.symlink_to(note_path.relative_to(legacy_note.parent))
    if compression_note_text is not None:
        legacy_compression = LEGACY_DOCS_DIR / compression_note_path.name
        if legacy_compression.exists() or legacy_compression.is_symlink():
            legacy_compression.unlink()
        legacy_compression.symlink_to(compression_note_path.relative_to(legacy_compression.parent))
    else:
        legacy_compression = LEGACY_DOCS_DIR / compression_note_path.name
        if legacy_compression.exists() or legacy_compression.is_symlink():
            legacy_compression.unlink()
